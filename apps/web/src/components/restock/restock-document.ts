// A4 print template for a restock list / purchase order - structurally
// mirrors apps/mobile/src/sales/invoiceHtml.ts's letterhead/table/totals
// layout (same brand colors, same @page A4 sizing), adapted for restock line
// items instead of sale line items. Web has no existing A4 template of its
// own (print-receipt.ts is a thermal-receipt layout only), so this is new
// rather than a port of something already here.

export type RestockDocumentLine = {
  productName: string;
  variantLabel: string | null;
  quantity: number;
  unitCost: number | null; // null when the viewer isn't allowed to see cost
};

export type RestockDocumentData = {
  poNumber: string;
  storeName: string;
  supplierName: string;
  createdAtLabel: string;
  lines: RestockDocumentLine[];
  estimatedTotal: number | null;
};

export type RestockDocumentTenant = {
  name: string;
};
...
function buildRestockDocumentHtml(data: RestockDocumentData, tenant: RestockDocumentTenant): string {
...
<body>
  ...
  <div class="parties">
    <div class="party">
      <h3>Store</h3>
      <div class="name">${escapeHtml(data.storeName)}</div>
    </div>
    <div class="party" style="text-align: right;">
      <h3>Supplier</h3>
      <div class="name">${escapeHtml(data.supplierName)}</div>
    </div>
  </div>
  ...
  ${
    ...
    <div class="totals">
      <div class="row grand"><span>Estimated total</span><span>${data.estimatedTotal.toFixed(2)}</span></div>
    </div>
  </div>`
      : ''
  }
...
export function printRestockDocument(data: RestockDocumentData, tenant: RestockDocumentTenant): boolean {
  const win = window.open('', '_blank', 'width=900,height=1000');
  if (!win) return false;

  const html = buildRestockDocumentHtml(data, tenant);
  win.document.open();
