...
export function NewProductDialog() {
  ...
  const [form, setForm] = useState({ sku: '', barcode: '', name: '', category: '', costPrice: '', sellPrice: '', taxRate: '0.16' });
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    if (!response.ok) {
      ...
      setError(body?.errors?.[0]?.message ?? 'Failed to create product');
      setLoading(false);
      return;
    }

    setOpen(false);
    setForm({ sku: '', barcode: '', name: '', category: '', costPrice: '', sellPrice: '', taxRate: '0.16' });
    setLoading(false);
    router.refresh();
  }
