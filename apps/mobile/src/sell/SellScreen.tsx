    ...
    <SafeAreaView edges={['top']} className="flex-1 bg-background">
      ...
      {results.length > 0 ? (
        ...
        <View className="flex-1 items-center justify-center px-6">
          <Text className="text-center font-medium text-foreground">{trimmedQuery ? `No products match "${trimmedQuery}"` : 'Ready to sell'}</Text>
          <Text className="mt-1 text-center text-muted-foreground">
            {trimmedQuery ? 'Try a different name, SKU, or scan the barcode directly.' : 'Scan a barcode or start typing to add a product to the sale.'}
          </Text>
        </View>
      )}
