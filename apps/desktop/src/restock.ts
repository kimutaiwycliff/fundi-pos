...
// Restocking is online-only by design on every client (see apps/mobile/src/
// restock/purchaseOrders.ts's own header comment - purchase-orders/suppliers
// aren't in PowerSync's sync-config.yaml bucket list), so this talks straight
// to the API via tauriFetch, mirroring that file's exact calls/shapes.
...
// local PowerSync query - desktop's local schema.ts has no reorder_point/
