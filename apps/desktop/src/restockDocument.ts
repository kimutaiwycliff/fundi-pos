// A4 print template for a restock list, ported verbatim from
// apps/web/src/components/restock/restock-document.ts /
// apps/mobile/src/restock/restockListHtml.ts (same letterhead/table/totals
// layout, same brand colors). Printed via a hidden same-page iframe rather
// than web's window.open('_blank', ...): Tauri's single-window webview
// doesn't reliably support real popup windows, but window.print() on the
// existing webview is confirmed unblocked (see the plan's own research) -
// an iframe gives isolated print-only content without needing a new window.

export interface RestockDocumentLine {
  productName: string;
...
export interface RestockDocumentData {
  poNumber: string;
  storeName: string;
  supplierName: string;
...
function buildRestockDocumentHtml(data: RestockDocumentData, tenantName: string): string {
...
export function printRestockDocument(data: RestockDocumentData, tenantName: string): void {
  ...
  const cleanup = () => {
    ...
    // (and the document it's printing) disappears from under it.
  ...
  iframe.srcdoc = buildRestockDocumentHtml(data, tenantName);
}
